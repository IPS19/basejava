...
public class SqlStorage implements Storage {
    ...
    private void insertSections(Connection conn, Resume r) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement("INSERT INTO section (resume_uuid, type, content) VALUES (?,?,?)")) {
            for (Map.Entry<SectionType, Sections> e : r.getSections().entrySet()) {
                ps.setString(1, r.getUuid());
                ps.setString(2, e.getKey().name());
                Sections section = e.getValue();
                ps.setString(3, JsonParser.write(section, Sections.class));
                ps.addBatch();
            }
            ps.executeBatch();
        }
    ...
    private void addContact(ResultSet rs, Resume r) throws SQLException {
        ...
        if (value != null) {
            r.setSection(ContactType.valueOf(rs.getString("type")), value);
        }
    }
    ...
    private void addSection(ResultSet rs, Resume r) throws SQLException {
        ...
        if (content != null) {
            ...
            r.setContact(type, JsonParser.read(content, Sections.class));
        }
    }
}